...
-- supplies & total quantity ordered from them
SELECT suppliers.supplier_name, SUM(orders.quantity_ordered) as supp_quantity
FROM suppliers
INNER JOIN orders on suppliers.supplier_id = orders.supplier_id
GROUP BY suppliers.supplier_name
ORDER BY supp_quantity DESC;

-- products with highest unit price
SELECT products.product_name, products.unit_price
FROM products
ORDER BY unit_price DESC
LIMIT 1;

-- orders with associated supplier & product info
SELECT p.*, suppliers.supplier_name, orders.order_id
FROM products as p
INNER JOIN orders on p.product_id = orders.product_id
RIGHT OUTER JOIN suppliers on orders.supplier_id = suppliers.supplier_id
GROUP BY suppliers.supplier_name, orders.order_id;

-- top 3 products with highest ordered_quantity
SELECT products.product_name, SUM(orders.quantity_ordered) as total_quantity
FROM products
INNER JOIN orders on products.product_id = orders.product_id
GROUP BY products.product_name
...
-- percentage of completed orders of total orders (alternate completion rows should return 50%)
SELECT (COUNT(CASE WHEN order_status = 'Completed' THEN 1 END) / COUNT(*)) * 100 as completion_rate
FROM orders;

-- total shipments per shipping company
SELECT shipments.shipping_company, COUNT(shipments.shipment_id) as shipments
FROM shipments
GROUP BY shipments.shipping_company
ORDER BY shipments DESC;

-- suppliers who haven't yet made an order (to verify query we can check POC)
SELECT suppliers.supplier_name, orders.order_id, suppliers.contact_person
FROM suppliers
LEFT OUTER JOIN orders on suppliers.supplier_id = orders.supplier_id
WHERE orders.order_id IS NULL;

-- orders with corresponding shipment details (no nulls)
SELECT orders.order_id, orders.quantity_ordered, s.*
FROM orders
JOIN shipments s on orders.order_id = s.order_id;

-- top 5 suppliers with highest total quantity ordered
SELECT suppliers.supplier_name, SUM(orders.quantity_ordered) as total_quantity
FROM suppliers
...
SELECT (quantity_in_stock + orders.quantity_ordered)*unit_price as revenue, suppliers.supplier_name
FROM products
INNER JOIN orders on products.product_id = orders.product_id
INNER JOIN suppliers on orders.supplier_id = suppliers.supplier_id
ORDER BY revenue DESC;
...
-- avg. delivery time
SELECT AVG(DATEDIFF(shipments.delivery_date, shipments.shipment_date)) as avg_del_time
FROM shipments;

-- products that were never ordered (there are no nulls in products or orders table so no result)
SELECT products.product_name
FROM products
INNER JOIN orders on products.product_id = orders.product_id
WHERE orders.product_id IS NULL;

-- top 3 shipping companies by #of shipments
SELECT shipments.shipping_company, COUNT(shipments.shipment_id) as deliveries
FROM shipments
GROUP BY shipments.shipping_company
ORDER BY deliveries DESC
LIMIT 3;

-- products with low inventory levels (<110)
SELECT products.product_id, products.product_name, products.quantity_in_stock
FROM products
WHERE quantity_in_stock <= 110
ORDER BY products.quantity_in_stock;

-- top 5 suppliers with revenue
SELECT (quantity_in_stock + orders.quantity_ordered)*unit_price as revenue, suppliers.supplier_name
FROM products
INNER JOIN orders on products.product_id = orders.product_id
INNER JOIN suppliers on orders.supplier_id = suppliers.supplier_id
